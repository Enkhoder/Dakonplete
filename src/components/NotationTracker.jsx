...
export default function NotationTracker() {
  ...
  const [pendingImport, setPendingImport] = useState(null); // { text, fromFile }
  ...
  // The snapIdx of the exact last played move part in the current active timeline.
  // This index corresponds to the result state in the undoStack.
  const livePositionIdx = undoStack.length - 1;

  const targetSnapIdx = previewIndex !== null ? previewIndex : livePositionIdx;

  const isPrevDisabled = isUndoing || undoStack.length === 0 || previewIndex === 0 || (!isImportReview && previewIndex === null && undoStack.length === 1);
  const isNextDisabled = isUndoing || undoStack.length === 0 || (previewIndex === null && !isImportReview) || (previewIndex === undoStack.length - 1 && isImportReview);
  ...
  // Handle current ongoing turn representation
  ...
  // Track the active cell index for the faint amber highlight
  // It should always match the cell containing the currently highlighted move part.
  let activeCellIdx = -1;
  if (targetSnapIdx === -1) {
  ...
  // Fallback for edge cases where the move is part of the ongoing turn
  ...
  const handleDownload = () => {
    ...
    const timestamp = `${d.getUTCFullYear()}${pad(d.getUTCMonth() + 1)}${pad(d.getUTCDate())}_${pad(d.getUTCHours())}${pad(d.getUTCMinutes())}${pad(d.getUTCSeconds())}`;
  ...
  // Reactive text box sync: rebuild the canonical display string whenever
  // the game's notation state changes (moves, undo, redo, reset, import).
  // Only overwrite localManualText when it still matches the last known
  // canonical value (i.e. the user hasn't started a manual edit).
  useEffect(() => {
  ...
  // Reset shake trigger when manual input closes
  useEffect(() => {
    if (!showManualInput) setShakeTrigger(0);
  }, [showManualInput]);

  // Reset custom undo/redo stack when manual input opens
  useEffect(() => {
    if (showManualInput) {
  ...
  // Enter answers the capture prompt with Yes, Escape with No
  ...
  // Number keys 1-7 sow the current player's matching cup (P1: 1-7, P2: 8-14)
  ...
  const processImport = (text, fromFile = false) => {
    ...
    // If game is in progress (and we aren't just updating a review), ask to reset first
  ...
  const performImport = (text, fromFile = false) => {
    ...
    // Guard: reject blank / whitespace-only input
    if (!text || !text.trim()) {
  ...
  return (
    <div className="w-full max-w-lg mt-4 bg-white border-2 border-black shadow-[4px_4px_0_#000]">
      {/* ----- Import reset confirmation (reusing style from gamemode change) ----- */}
      {pendingImport && (
        <div className="fixed inset-0 bg-black/20 z-[300] flex items-center justify-center">
          <div className="bg-amber-50 border-2 border-black p-5 shadow-[4px_4px_0_#000] w-64 text-center">
            ...
            <div className="flex gap-2">
              <button onClick={() => setPendingImport(null)} className="flex-1 py-1.5 text-xs font-bold border-2 border-black bg-white hover:bg-stone-100 transition-colors">Cancel</button>
              <button onClick={() => { performImport(pendingImport.text, pendingImport.fromFile); setPendingImport(null); }} className="flex-1 py-1.5 text-xs font-bold border-2 border-black bg-red-400 text-white hover:bg-red-500 transition-colors">Confirm</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-between items-center p-3 border-b-2 border-black bg-stone-50 flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          {(previewIndex !== null && !isImportReview) && (
            <button
              ...
              className={`flex items-center gap-1.5 px-2 py-1 text-[11px] font-bold bg-amber-400 border-2 border-black text-black hover:bg-amber-500 transition-colors ${shakeActive ? 'wrong-click-shake' : ''}`}
            >
        ...
        <div className="flex gap-2">
          ...
          <div className="relative group">
            ...
            {notations.length > 0 && (
              <div className="absolute right-0 top-full mt-1 w-36 bg-white border-2 border-black shadow-[3px_3px_0_#000] flex opacity-0 invisible group-hover:visible group-hover:opacity-100 transition-all group-hover:delay-0 flex-col z-50 after:absolute after:-top-2 after:left-0 after:h-2 after:w-full">
                <button
            ...
            <AnimatePresence mode="wait">
              {activeToast && (
                <motion.div
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 text-white text-xs font-bold border-2 border-black shadow-[3px_3px_0_#000] ${activeToast.type === 'error' ? 'bg-red-600' : 'bg-emerald-700'}`}
                >
          ...
          {/* Import Group */}
          <div className="relative group">
            <button className="flex items-center gap-1 px-3 py-1 text-[11px] font-bold bg-black text-white border-2 border-black group-hover:bg-emerald-800 transition-colors">
              <Upload size={12} />
              Import
            </button>
            <div className="absolute right-0 top-full mt-1 w-36 bg-white border-2 border-black shadow-[3px_3px_0_#000] flex opacity-0 invisible group-hover:visible group-hover:opacity-100 transition-all group-hover:delay-0 flex-col z-50 text-black after:absolute after:-top-2 after:left-0 after:h-2 after:w-full">
              <button
        ...
        <div className="p-3 border-b-2 border-black bg-amber-50">
          ...
          >
            <textarea
              ...
              spellCheck="false"
            />
            {/* Custom scroll-immune placeholder label */}
            {(placeholderOverride || !localManualText) && (
              <div className="absolute top-2.5 left-2.5 text-sm text-gray-400 pointer-events-none z-20" style={{ fontFamily: '"Consolas", monospace' }}>
            ...
            <button
              onClick={() => {
                ...
                if (isMeaningful) {
                  ...
                  // Trigger the "Nothing to clear" effect
                  if (placeholderTimerRef.current) clearTimeout(placeholderTimerRef.current);
              ...
              <button
                onClick={() => {
                  ...
                  if (isMeaningful) {
                    ...
                    // Let performImport handle closing the modal on success
                  } else {
                    if (placeholderTimerRef.current) clearTimeout(placeholderTimerRef.current);
          ...
          {showClearConfirm && (
            <div className="mt-3 bg-white border-2 border-black p-3 shadow-[3px_3px_0_#000] text-center animate-in fade-in zoom-in duration-200">
              <p className="text-xs font-bold text-gray-700 mb-2">Clear all manual input? This won't affect the game.</p>
              <div className="flex gap-2">
                <button onClick={() => setShowClearConfirm(false)} className="flex-1 py-1 text-xs font-bold border-2 border-black bg-white hover:bg-stone-100">Cancel</button>
                <button onClick={() => { setLocalManualText(''); setManualText(''); setShowClearConfirm(false); }} className="flex-1 py-1 text-xs font-bold border-2 border-black bg-red-400 text-white hover:bg-red-500 transition-colors">Clear</button>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="w-full p-0">
        <table className="w-full text-left text-sm cursor-default">
          <thead className="bg-stone-100 sticky top-0 border-b-2 border-black text-xs uppercase tracking-widest text-gray-500 font-bold z-10">
            <tr>
            ...
            {rows.map((row) => {
              ...
              return (
                <tr key={row.turnNum} className="border-b-[1px] border-gray-200 hover:bg-amber-50 transition-colors">
                  ...
                  <td
                    className={`px-4 py-2 border-r-2 border-black transition-colors ${row.p1.parts.length > 0 ? 'cursor-pointer hover:bg-yellow-100' : ''} ${p1Active ? 'bg-yellow-50' : ''}`}
                    onClick={() => handleCellClick(row.p1)}
                  >
                    {renderCell(row.p1, true)}
                  </td>
                  <td
                    className={`px-4 py-2 transition-colors ${row.p2.parts.length > 0 ? 'cursor-pointer hover:bg-yellow-100' : ''} ${p2Active ? 'bg-yellow-50' : ''}`}
